 ...
 * @author darraghd493
 */
public class MultiEnumPropertyElement<T extends Enum<T> & PropertyEnum> extends PropertyElement<MultiEnumProperty<T>, List<T>> {
    private boolean expanded;

    protected MultiEnumPropertyElement(MultiEnumProperty<T> property, BooleanSupplier visibility, int offset) {
    ...
    public int draw(@NotNull Screen screen, @NotNull GuiGraphics guiGraphics, Container container, int mouseX, int mouseY, float partialTick) {
        ...
        for (T value : this.property.getValues()) {
            guiGraphics.drawString(
                    ...
                    this.property.getValue().contains(value) ? 0xFFFFFF : 0x888888
            );
            guiGraphics.fill(
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (this.expanded && RenderUtil.isMouseInArea(mouseX, mouseY, this.x, this.y + HEIGHT, this.width, HEIGHT * (this.property.getValues().size() + 1))) {
            ...
            if (index >= 0 && index < this.property.getValues().size()) {
                this.property.toggle(this.property.getValues().get(index));
                return true;
            }
        }
        return false;
